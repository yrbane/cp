...
fn integ_single_file_matches_gnu() {
    ...
    StdCommand::new("/usr/bin/cp").arg(e.p("src")).arg(e.p("gnu")).output().unwrap();
...
fn integ_recursive_matches_gnu() {
    ...
    StdCommand::new("/usr/bin/cp").arg("-R").arg(e.p("src")).arg(e.p("gnu")).output().unwrap();
    cp().arg("-R").arg(e.p("src")).arg(e.p("our")).assert().success();
    ...
    assert_eq!(link_target(&e.p("gnu/a/link")), link_target(&e.p("our/a/link")));
}
...
fn integ_preserve_matches_gnu() {
    ...
    StdCommand::new("/usr/bin/cp").arg("-p").arg(e.p("src")).arg(e.p("gnu")).output().unwrap();
    cp().arg("-p").arg(e.p("src")).arg(e.p("our")).assert().success();
...
fn integ_error_same_file() {
    ...
    cp().arg(e.p("f")).arg(e.p("f")).assert().failure()
        .stderr(predicates::str::contains("same file")).code(1);
}
...
fn integ_error_omit_directory() {
    ...
    cp().arg(e.p("dir")).arg(e.p("dst")).assert().failure()
        .stderr(predicates::str::contains("omitting directory")).code(1);
}
...
fn integ_error_not_a_directory() {
    ...
    cp().arg(e.p("a")).arg(e.p("b")).arg(e.p("c")).assert().failure()
...
fn integ_verbose_format() {
    ...
    let out = cp().arg("-v").arg(e.p("src")).arg(e.p("dst")).output().unwrap();
...
fn integ_exit_code_success() {
    ...
    cp().arg(e.p("src")).arg(e.p("dst")).assert().success().code(0);
}

#[test]
fn integ_exit_code_failure() {
    cp().arg("/nonexistent").arg("/tmp/whatever").assert().failure().code(1);
}
...
fn integ_multiple_sources_to_dir() {
    ...
    for i in 0..5 {
        assert_eq!(content(&e.p(&format!("dest/file_{i}"))), format!("content {i}"));
    }
}
